...
const categories = ['All Categories', 'Entertainment', 'Education', 'News', 'Sports', 'Music', 'Gaming', 'Technology', 'Lifestyle', 'Health', 'Science', 'Art', 'Comedy'];
export function SponsorOnboarding() {
  ...
  const fetchAvailableVideos = async () => {
    try {
      const {
        ...
        `).eq('distribution_type', 'public').eq('revenue_enabled', true);
  ...
  const handleFileUpload = async (file: File, type: 'video' | 'banner') => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.storage.from('ad-assets').upload(fileName, file);
      if (error) throw error;
      const {
        data: {
          publicUrl
        }
      } = supabase.storage.from('ad-assets').getPublicUrl(data.path);
      if (type === 'video') {
        setCampaign(prev => ({
          ...prev,
          adVideoUrl: publicUrl
        }));
      } else {
        setCampaign(prev => ({
          ...prev,
          adBannerUrl: publicUrl
        }));
      }
      toast({
  ...
  const submitCampaign = async () => {
    try {
      ...
      let {
        data: sponsor
      } = await supabase.from('sponsors').select('id').eq('user_id', userData.id).single();
      if (!sponsor) {
        const {
          data: newSponsor,
          error: sponsorError
  ...
  const filteredVideos = availableVideos.filter(video => campaign.targetCategory === 'All Categories' || video.category === campaign.targetCategory);
  ...
  return <div className="max-w-4xl mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Create targeted ad campaigns for Renkiva offline network</CardDescription>
          <div className="flex items-center gap-2 mt-4">
            {[1, 2, 3, 4].map(stepNum => <div key={stepNum} className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${step >= stepNum ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                {stepNum}
              </div>)}
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          {step === 1 && <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="campaign-name" value={campaign.campaignName} onChange={e => setCampaign(prev => ({
                ...prev,
                ...
                <div className="space-y-2">
                  ...
                  <Select value={campaign.format} onValueChange={(value: any) => setCampaign(prev => ({
                ...prev,
                format: value
              }))}>
                    <SelectTrigger>
                ...
                {(campaign.format === 'video' || campaign.format === 'both') && <div className="space-y-2">
                    <Label htmlFor="video-upload">Ad Video (5-15 seconds)</Label>
                    <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center">
                      <Play className="h-12 w-12 mx-auto mb-2 text-muted-foreground" />
                      <Input id="video-upload" type="file" accept="video/*" onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) handleFileUpload(file, 'video');
                }} className="hidden" />
                      <Button variant="outline" disabled={uploading} onClick={() => document.getElementById('video-upload')?.click()} type="button">
                        {uploading ? 'Uploading...' : 'Choose Video Ad'}
                      </Button>
                      {campaign.adVideoUrl && <p className="text-sm text-green-600 mt-2">Video uploaded ✓</p>}
                    </div>
                  </div>}

                {(campaign.format === 'banner' || campaign.format === 'both') && <div className="space-y-2">
                    <Label htmlFor="banner-upload">Banner Image</Label>
                    <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center">
                      <ImageIcon className="h-12 w-12 mx-auto mb-2 text-muted-foreground" />
                      <Input id="banner-upload" type="file" accept="image/*" onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) handleFileUpload(file, 'banner');
                }} className="hidden" />
                      <Button variant="outline" disabled={uploading} onClick={() => document.getElementById('banner-upload')?.click()} type="button">
                        {uploading ? 'Uploading...' : 'Choose Banner'}
                      </Button>
                      {campaign.adBannerUrl && <p className="text-sm text-green-600 mt-2">Banner uploaded ✓</p>}
                    </div>
                  </div>}

                <div className="space-y-2">
                  <Label htmlFor="runtime">Video Runtime (seconds)</Label>
                  <Input id="runtime" type="number" min="5" max="30" value={campaign.runtimeSeconds} onChange={e => setCampaign(prev => ({
                ...prev,
            ...
            setStep(2);
          }} className="w-full" disabled={!campaign.campaignName || campaign.format === 'video' && !campaign.adVideoUrl || campaign.format === 'banner' && !campaign.adBannerUrl || campaign.format === 'both' && (!campaign.adVideoUrl || !campaign.adBannerUrl)}>
          ...
          {step === 2 && <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select value={campaign.targetCategory} onValueChange={value => setCampaign(prev => ({
                ...prev,
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-2 border rounded-lg p-4">
                    {filteredVideos.length === 0 ? <p className="text-muted-foreground text-center py-4">
                        No videos available for the selected category
                      </p> : filteredVideos.map(video => <div key={video.id} className={`p-3 border rounded-lg cursor-pointer transition-colors ${selectedVideos.includes(video.id) ? 'bg-primary/10 border-primary' : 'hover:bg-muted/50'}`} onClick={() => toggleVideoSelection(video.id)}>
                          <div className="flex justify-between items-start">
                            ...
                            {selectedVideos.includes(video.id) && <Badge variant="default">Selected</Badge>}
                          </div>
                        </div>)}
                  </div>
                  {selectedVideos.length > 0 && <p className="text-sm text-muted-foreground">
          ...
          {step === 3 && <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="budget" type="number" min="10" step="10" value={campaign.budgetAllocated} onChange={e => setCampaign(prev => ({
                ...prev,
                ...
                <div className="space-y-2">
                  ...
                  <Input id="cost-per-impression" type="number" min="0.01" step="0.01" max="1.00" value={campaign.costPerImpression} onChange={e => setCampaign(prev => ({
                ...prev,
              ...
              <div className="p-4 bg-muted rounded-lg">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {selectedVideos.length > 0 && <p>Targeting: <span className="font-medium">{selectedVideos.length} specific videos</span></p>}
                  {campaign.targetCategory !== 'All Categories' && <p>Category: <span className="font-medium">{campaign.targetCategory}</span></p>}
                </div>
              </div>
          ...
          {step === 4 && <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={submitCampaign} className="flex-1" disabled={uploading}>
