...
const CapabilitiesSection = () => {
  ...
  const differentiators = [
    ...
    "📡 True Mesh Networking - Not just hotspot sharing",
    "🎧 Podcast-Optimized - Upload and stream audio content offline",
    "🔐 Privacy-First - No data collection or tracking",
    "⚡ Battery Efficient - Bluetooth LE for extended usage",
    "🎬 Legal Content - Built-in access to public domain movies",
    "🌐 Community Driven - Share content with nearby users",
    "🔄 Sync-Ready - Upload watch data when back online"
  ];
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {coreFeatures.map((feature, index) => (
            <Card 
              key={index} 
              className={`bg-card/80 backdrop-blur-lg border-border/50 shadow-clay transition-all duration-300 hover:shadow-clay-lg ${
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {differentiators.map((point, index) => (
              <div 
                key={index}
                className="bg-card/60 backdrop-blur-lg border border-border/30 rounded-lg p-4 text-center shadow-clay-inset hover:bg-card/80 transition-colors"
              >
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {technicalFeatures.map((feature, index) => (
              <Card key={index} className="bg-card/60 backdrop-blur-lg border-border/30 shadow-clay-inset">
